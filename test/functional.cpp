...
TEST_CASE("Functional - mapf range")
{
    ...
    auto result = utils::functional::mapf([](int x) { return x * 2; }, v);
...
TEST_CASE("Functional - mapf queue")
{
    ...
    q.push(2);
    q.push(3);
    auto result = utils::functional::mapf([](int x) { return x + 10; }, q);
...
TEST_CASE("Functional - foldl")
{
    ...
    auto sum = utils::functional::foldl(
        [](int acc, int x) { return acc + x; }, v, 0);
    REQUIRE(sum == 15);
}
...
TEST_CASE("Functional - foldr")
{
    ...
    auto result = utils::functional::foldr(
        [](std::string const& a, std::string const& b) { return a + b; }, v,
        std::string{});
    REQUIRE(result == "abc");
}
...
TEST_CASE("Functional - foldl queue (by value, non-destructive to caller)")
{
    ...
    q.push(3);
    auto sum = utils::functional::foldl(
        [](int acc, int x) { return acc + x; }, q, 0);
    REQUIRE(sum == 6);
...
TEST_CASE("Functional - map / filter transducers")
{
    ...
    auto xf = utils::functional::filter([](int x) { return x % 2 == 0; })(
        utils::functional::map([](int x) { return x * 2; })(push));

    std::vector<int> result;
    for (int x : v) { result = xf(result, x); }
...
TEST_CASE("Functional - combine applies two functions and merges results")
{
    ...
    auto f = utils::functional::combine(
        [](int a, int b) { return a + b; }, square, negate);
    REQUIRE(f(3) == 6);
    REQUIRE(f(5) == 20); // 25 + (-5)
}
